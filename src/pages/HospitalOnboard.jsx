...
const HospitalOnboard = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`${apiUrl}/api/${apiVersion}/hospital/onboard`, {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          ...formData,
          username: formData.phone // Use phone as username if not provided
        })
      });
      const data = await response.json();
      if (data.success) {
        setSuccess(true);
        setFormData({ name: '', email: '', phone: '', username: '', password: '', role: 'doctor' });
      } else {
  ...
  return (
    <div className="p-6 lg:p-10 max-w-2xl mx-auto space-y-8 animate-in slide-in-from-bottom-8 duration-500">
      <div>
        ...
        <h1 className="text-3xl font-black text-slate-900 tracking-tight">Onboard Staff / Patient</h1>
        <p className="text-slate-500 font-medium">Directly register medical staff or patients linked to your hospital.</p>
      </div>
