...
const Messages = () => {
  ...
  const [chatPrescriptions, setChatPrescriptions] = useState([]);
  const [prescriptionModalMode, setPrescriptionModalMode] = useState('list'); // 'list' or 'create'
  ...
  const handleSelectChat = async (consultation) => {
    ...
    try {
      ...
      const pxResponse = await fetch(`${apiUrl}/api/${apiVersion}/prescriptions?consultationId=${consultation._id}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      const pxData = await pxResponse.json();
      if (pxData.success) {
        setChatPrescriptions(pxData.prescriptions || []);
      } else {
        setChatPrescriptions([]);
      }
    } catch (error) {
  ...
  const startCall = async (type) => {
    ...
    try {
      ...
      peer.on('signal', (data) => {
        console.log(
          '📤 SIGNAL OUT:',
          data.type || (data.candidate ? 'candidate' : 'sdp')
        );
      ...
      peer.on('track', (track, incomingStream) => {
        ...
        if (track.kind === 'video') {
          const trackSettings = incomingStream.getVideoTracks()?.[0]?.getSettings();
          if (trackSettings) {
            console.log('REMOTE VIDEO DIMENSIONS:', trackSettings.width, trackSettings.height);
            setRemoteVideoPortrait(trackSettings.height > trackSettings.width);
          }
        }
  ...
  const answerCall = async () => {
    ...
    try {
      ...
      if (pendingSignals.current.length > 0) {
        console.log(
          `Processing ${pendingSignals.current.length} queued signals...`
        );
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.success && data.prescription) {
          setChatPrescriptions(prev => [data.prescription, ...prev]);
        }
  ...
  return (
    <main className="flex-1 lg:ml-[280px] h-screen max-h-screen flex bg-white font-['Poppins'] overflow-hidden relative">
              ...
              return (
                ...
                >
                  <div className="relative">
                    ...
                    {((other?.role === 'doctor' || other?.doctorDetails) ? other?.doctorDetails?.isOnline : false) && (
            ...
            <header className="px-4 lg:px-8 py-4 bg-white border-b border-slate-100 flex items-center justify-between shadow-sm z-10 flex-shrink-0">
              <div className="flex items-center gap-3 lg:gap-4">
                ...
                <div className="relative">
                  ...
                  {((isDoctor ? selectedChat.patient : selectedChat.doctor)?.role === 'doctor' || (isDoctor ? selectedChat.patient : selectedChat.doctor)?.doctorDetails) ? (
                    (isDoctor ? selectedChat.patient : selectedChat.doctor)?.doctorDetails?.isOnline && (
                    ...
                    {showMoreMenu && (
                      <div className="absolute right-0 mt-2 w-56 bg-white rounded-2xl shadow-2xl border border-slate-100 py-2 z-[60] animate-in fade-in slide-in-from-top-2 duration-200">
                        <button
                          ...
                          onClick={() => {
                            if (chatPrescriptions.length > 0) {
                              setPrescriptionModalMode('list');
                            } else {
                        ...
                        >
                          ...
                          {chatPrescriptions.length > 0 ? 'View Prescriptions' : 'Issue Prescription'}
                        </button>
      ...
      >
        {callType === 'video' ? (
          <>
            {/* Remote Video Container - Professional Aspect Ratio Handling */}
            <div className={`relative flex items-center justify-center overflow-hidden bg-black transition-all duration-500 w-full h-full ${remoteVideoPortrait ? 'md:max-w-[450px] md:mx-auto shadow-2xl border-x border-white/5' : ''}`}>
              <video
                ref={remoteVideoRef}
                autoPlay
                playsInline
            ...
            {/* Local Self-Preview - Uses object-cover for mirror effect */}
          ...
          <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl">
            <div className="px-6 py-4 border-b flex items-center justify-between bg-slate-50">
              <h3 className="font-bold text-slate-800">
                {prescriptionModalMode === 'list' ? 'Patient Prescriptions' : 'Issue New Prescription'}
              </h3>
              <button onClick={() => setShowPrescriptionModal(false)} className="text-slate-400 hover:text-slate-600 font-bold text-lg">✕</button>
            </div>
            
            {prescriptionSuccess ? (
              <div className="p-12 text-center text-emerald-600 font-bold flex flex-col items-center gap-2 animate-in fade-in">
                <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" className="text-emerald-500 mb-2">
                  <polyline points="20 6 9 17 4 12"></polyline>
                </svg>
                Prescription Sent Successfully!
              </div>
            ) : prescriptionModalMode === 'list' ? (
              <div className="p-6 overflow-y-auto flex flex-col gap-5 max-h-[70vh]">
                <div className="flex justify-between items-center pb-2 border-b border-slate-100">
                  <span className="text-xs text-slate-500 font-bold uppercase tracking-wider">
                    {chatPrescriptions.length} Prescription{chatPrescriptions.length !== 1 ? 's' : ''} Record{chatPrescriptions.length !== 1 ? 's' : ''}
                  </span>
                  <button
                    type="button"
                    onClick={() => setPrescriptionModalMode('create')}
                    className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-500 hover:bg-emerald-600 text-white rounded-lg text-xs font-bold transition-all shadow-md shadow-emerald-100"
                  >
                ...
                <div className="flex flex-col gap-4">
                  {[...chatPrescriptions].sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)).map((px, index) => (
                    <div key={px._id || index} className="border border-slate-200 rounded-xl bg-slate-50/50 overflow-hidden shadow-sm">
                      <div className="bg-slate-100/70 px-4 py-2.5 flex justify-between items-center border-b border-slate-200">
                        <div className="text-xs font-bold text-slate-700">
                          Prescription #{chatPrescriptions.length - index}
                        </div>
                        <div className="text-[11px] font-semibold text-slate-500">
                          {new Date(px.createdAt).toLocaleString(undefined, {
                            year: 'numeric',
                            month: 'short',
                            day: 'numeric',
                            hour: '2-digit',
                      ...
                      <div className="p-4 flex flex-col gap-3.5">
                        <div>
                          <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Observation / Diagnosis</div>
                          <div className="text-xs text-slate-800 font-medium bg-white p-3 rounded-lg border border-slate-200/60 leading-relaxed shadow-sm">{px.diagnosis}</div>
                        </div>

                        {px.medicines && px.medicines.length > 0 && (
                          <div>
                            <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">Medicines</div>
                            <div className="flex flex-col gap-1.5">
                              {px.medicines.map((med, mIdx) => (
                                <div key={mIdx} className="flex flex-wrap items-center gap-x-2 text-xs text-slate-700 bg-white px-3 py-2.5 rounded-lg border border-slate-200/60 shadow-sm">
                                  <span className="w-4.5 h-4.5 bg-blue-50 text-blue-600 rounded-full flex items-center justify-center text-[9px] font-bold flex-shrink-0">{mIdx + 1}</span>
                                  ...
                                  {med.dosage && <span className="text-[11px] text-slate-500">— {med.dosage}</span>}
                                  {med.frequency && <span className="text-[11px] text-slate-500">— {med.frequency}</span>}
                                  {med.duration && <span className="text-[11px] text-slate-500">— {med.duration}</span>}
                                  {med.instruction && <div className="w-full text-[11px] text-amber-700 bg-amber-50 px-2.5 py-1.5 rounded-md mt-1.5 border border-amber-100/50 font-medium">📝 {med.instruction}</div>}
                                </div>
                              ))}
                            </div>
                          </div>
                        ...
                        {px.advice && (
                          <div>
                            <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Doctor's Advice</div>
                            <div className="text-xs text-emerald-800 bg-emerald-50/60 p-3 rounded-lg border border-emerald-100/50 leading-relaxed font-semibold shadow-sm">{px.advice}</div>
                          </div>
                        )}
                      </div>
                    </div>
              ...
              <form className="p-6 overflow-y-auto flex flex-col gap-5 max-h-[70vh]" onSubmit={handleSubmitPrescription}>
                {chatPrescriptions.length > 0 && (
                  <button
                    type="button"
                    onClick={() => setPrescriptionModalMode('list')}
                    className="flex items-center gap-1.5 text-xs text-slate-500 hover:text-slate-700 font-bold mb-1 transition-colors self-start bg-slate-100 hover:bg-slate-200 px-3 py-1.5 rounded-lg"
                  >
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
                      <line x1="19" y1="12" x2="5" y2="12"></line><polyline points="12 19 5 12 12 5"></polyline>
                    </svg>
                ...
                <div className="flex flex-col gap-1">
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Observation / Diagnosis</label>
                  <textarea
                    className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-sm focus:bg-white focus:border-blue-500 transition-all outline-none"
                    placeholder="Describe patient's condition, diagnosis or observations..."
                ...
                <div className="flex flex-col gap-2">
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Medicines</label>
                  {prescriptionMedicines.map((med, i) => (
                    <div key={i} className="flex gap-2 items-center bg-slate-50 p-2.5 rounded-xl border border-slate-200/50">
                      <input
                        className="flex-1 bg-white border border-slate-200 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 rounded-lg px-3 py-2 text-xs outline-none transition-all"
                      ...
                      <button
                        ...
                        className="w-8 h-8 rounded-lg bg-red-50 text-red-500 hover:bg-red-100 transition-all flex items-center justify-center font-bold text-xs"
                      >
                        ✕
                      </button>
                    </div>
                  ...
                  <button
                    type="button"
                    className="text-blue-600 hover:text-blue-700 text-xs font-bold flex items-center gap-1 mt-1 self-start"
                ...
                <div className="flex flex-col gap-1">
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Doctor's Advice (Optional)</label>
                  <textarea
                    className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-sm focus:bg-white focus:border-blue-500 transition-all outline-none"
                    placeholder="Dietary instructions, rest, precautions, etc..."
                ...
                <button
                  type="submit"
                  className="w-full bg-emerald-500 hover:bg-emerald-600 text-white py-3 rounded-xl font-bold shadow-lg transition-all disabled:opacity-50 mt-2"
