...
function DoctorDashboard() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const storiesRes = await fetch(`${apiUrl}/api/${apiVersion}/stories`);
      ...
      <div className="px-8">
        ...
        <div className="grid grid-cols-12 gap-6">
          ...
          <div className="col-span-12 lg:col-span-8 space-y-6">
            ...
            {/* Health Stories Tray */}
            {stories.length > 0 && (
              <section className="bg-white border border-clinical-outline-variant/30 rounded-[32px] p-6 shadow-sm animate-in fade-in duration-500">
                <div className="flex items-center gap-2 mb-4">
                  <span className="material-symbols-outlined text-clinical-primary">history_toggle_off</span>
                  <h3 className="text-base font-bold text-clinical-primary">New Stories</h3>
                </div>
                <div className="flex gap-6 overflow-x-auto pb-2 scrollbar-hide">
                  {stories.map((group, groupIdx) => (
                    ...
                    >
                      <div className="w-16 h-16 rounded-full p-[3px] bg-gradient-to-tr from-blue-500 via-purple-500 to-pink-500 group-hover:scale-105 transition-all shadow-md">
                        <div className="w-full h-full rounded-full bg-white p-[2px]">
                          {group.author.profile?.avatar ? (
                            <img
                              src={group.author.profile.avatar.startsWith('http') || group.author.profile.avatar.startsWith('data:') ? group.author.profile.avatar : `${apiUrl}${group.author.profile.avatar}`}
                              alt=""
                      ...
                      <span className="text-xs font-bold text-slate-700 max-w-[80px] truncate group-hover:text-clinical-primary transition-colors">
              ...
              <div className="space-y-3">
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <span className="text-sm font-bold text-clinical-primary">Health Stories</span>
                  </div>
                  <span className="material-symbols-outlined text-clinical-on-surface-variant/40 group-hover:translate-x-1 transition-transform">
                    chevron_right
                  </span>
                ...
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full border-2 border-white/50 overflow-hidden flex-shrink-0 bg-slate-800">
                    {stories[activeStoryAuthorIndex].author.profile?.avatar ? (
                      <img 
                        src={stories[activeStoryAuthorIndex].author.profile.avatar.startsWith('http') || stories[activeStoryAuthorIndex].author.profile.avatar.startsWith('data:') ? stories[activeStoryAuthorIndex].author.profile.avatar : `${apiUrl}${stories[activeStoryAuthorIndex].author.profile.avatar}`} 
                        alt="" 
                        className="w-full h-full object-cover"
                      />
                    ) : (
